...
def getmultiples(num, lowerbound, upperbound, addnum):
    """
    This function finds all the factors of 256 between two given bounds, then adds a supplied number
    """
...
def answer(digest):
    ...
    for i in range(len(digest)):
        ...
        for multiple in getmultiples(256, 0, 256*129, digest[i]):
            if (multiple ^ previous) % 129 == 0:
                message.append(((multiple ^ previous) // 129) % 256)
    return message
